...
constexpr std::array<char, 8> kRequestMagic = {'O', 'T', 'R', 'L', 'P', 'S', '0', '1'};
constexpr std::array<char, 8> kResponseMagic = {'O', 'T', 'R', 'L', 'P', 'R', '0', '1'};
constexpr std::uint32_t kAct = 1;
constexpr std::uint32_t kUpdate = 2;
constexpr std::uint32_t kCheckpoint = 3;
...
public:
    ...
    std::string string()
    {
        ...
        if (length > 65535U) throw std::length_error("service string exceeds bound");
        require(length);
...
bool read_exact_or_eof(int descriptor, void *buffer, std::size_t length)
{
    ...
    while (received < length) {
        ...
        if (result < 0) throw std::runtime_error("trainer service read failed: " + std::string(std::strerror(errno)));
        if (result == 0) {
            if (received == 0) return false;
            throw std::runtime_error("trainer service frame header was truncated");
        }
...
void write_exact(int descriptor, const void *buffer, std::size_t length)
{
    ...
    while (written < length) {
        ...
        if (result < 0 && errno == EINTR) continue;
        if (result <= 0) throw std::runtime_error("trainer service write failed: " + std::string(std::strerror(errno)));
...
torch::Tensor read_observations(Reader &reader, std::int64_t samples)
{
    auto result = torch::empty({samples, kStructuredFeatures}, torch::kFloat32);
    auto view = result.accessor<float, 2>();
    for (std::int64_t sample = 0; sample < samples; ++sample) {
        for (std::int64_t feature = 0; feature < kStructuredFeatures; ++feature) view[sample][feature] = reader.f32();
    }
    return result;
}
...
{
    auto result = torch::empty({samples, kActionCount}, torch::kBool);
    auto view = result.accessor<bool, 2>();
    for (std::int64_t sample = 0; sample < samples; ++sample) {
        for (std::int64_t action = 0; action < kActionCount; ++action) {
...
std::vector<std::uint8_t> handle_act(PpoTrainer &trainer, Reader &reader)
{
    ...
    const auto observations = read_observations(reader, samples);
    const auto masks = read_masks(reader, samples);
    reader.finish();
    const auto result = trainer.act(observations, masks, deterministic != 0U);
...
std::vector<std::uint8_t> handle_update(PpoTrainer &trainer, Reader &reader)
{
    const auto samples = static_cast<std::int64_t>(reader.u32());
    ...
    auto observations = read_observations(reader, samples);
    ...
    for (std::int64_t sample = 0; sample < samples; ++sample) {
        ...
        if (bootstrap_value > 1U || continuation_value > 1U) throw std::invalid_argument("service GAE mask is not boolean");
    ...
    const auto gae = compute_gae(
        rewards.reshape(shape), old_values.reshape(shape), next_values.reshape(shape),
        bootstrap.reshape(shape), continuation.reshape(shape), config.gamma, config.gae_lambda);
...
std::vector<std::uint8_t> handle_checkpoint(PpoTrainer &trainer, Reader &reader)
{
    ...
    Writer writer;
    writer.string(saved.checkpoint_id);
    ...
    const std::filesystem::path &diagnostic_root)
{
    if (!diagnostic_root.is_absolute()) {
        throw std::invalid_argument("trainer service diagnostic root must be absolute");
    }
    if constexpr (std::endian::native != std::endian::little) {
        throw std::runtime_error("trainer service V1 requires an explicitly supported little-endian host");
    }
    while (true) {
        std::array<std::uint8_t, 20> header{};
        if (!read_exact_or_eof(input_descriptor, header.data(), header.size())) return 0;
        if (std::memcmp(header.data(), kRequestMagic.data(), kRequestMagic.size()) != 0) {
            throw std::runtime_error("trainer service request magic mismatch");
        }
        const auto type = decode_u32(header.data() + 8);
        const auto length = decode_u64(header.data() + 12);
        if (length > kMaximumFrameBytes) throw std::length_error("trainer service request exceeds frame bound");
        std::vector<std::uint8_t> payload(static_cast<std::size_t>(length));
        if (!payload.empty() && !read_exact_or_eof(input_descriptor, payload.data(), payload.size())) {
            throw std::runtime_error("trainer service request body was truncated");
        }
        try {
            ...
            else if (type == kCheckpoint) response = handle_checkpoint(trainer, reader);
            else if (type == kExit) {
                reader.finish();
                send_response(output_descriptor, type, 0, {});
                return 0;
            } else throw std::invalid_argument("unknown trainer service request type");
            send_response(output_descriptor, type, 0, response);
        } catch (const std::exception &error) {
            const bool numerical_failure =
                std::string_view(error.what()).find("nonfinite") != std::string_view::npos;
            std::string error_message(error.what());
            if (numerical_failure) {
                try {
                    (void)write_numerical_diagnostic(
                        diagnostic_root,
                        trainer.counters(),
                        type == kUpdate ? "trainer-service-update" : "trainer-service-act",
                        error.what());
            ...
            send_response(output_descriptor, type, 1, writer.data());
