...
class Trainer {
public:
    Trainer(uint64_t seed, torch::Device device, int64_t rollout_length = 32) : device_(std::move(device)), rng_(seed), model_(rng_.initialization_seed())
    {
        if (device_.is_cuda() && !torch::cuda::is_available()) throw std::runtime_error("CUDA unavailable; no fallback");
        if (rollout_length != 32 && rollout_length != 64) throw std::invalid_argument("rollout length must be 32 or 64");
    ...
    void training_info() const
    {
        std::cout << "{\"rollout_steps\":" << config_.rollout_length << ",\"sequence_length\":" << kSequence
            << ",\"optimization_epochs\":" << config_.optimization_epochs << "}" << std::endl;
    }
...
{
    try {
        if ((argc != 5 && argc != 7) || std::string(argv[1]) != "--device" || std::string(argv[3]) != "--seed")
            throw std::invalid_argument("usage: --device cpu|cuda:0 --seed INTEGER [--rollout-length 32|64]");
        if (std::string(argv[2]) != "cpu" && std::string(argv[2]) != "cuda:0") throw std::invalid_argument("unsupported device");
        int64_t rollout_length = 32;
        if (argc == 7) {
            if (std::string(argv[5]) != "--rollout-length" || (std::string(argv[6]) != "32" && std::string(argv[6]) != "64"))
                throw std::invalid_argument("rollout length must be 32 or 64");
            rollout_length = std::string(argv[6]) == "64" ? 64 : 32;
        }
        ...
        Trainer trainer(std::stoull(argv[4]), torch::Device(argv[2]), rollout_length);
