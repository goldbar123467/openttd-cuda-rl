...
public:
    Trainer(uint64_t seed, torch::Device device, int64_t rollout_length = 32, double gae_lambda = 0.95,
        dev::FinancialFeatures financial_features = dev::FinancialFeatures::Raw) :
...
{
    try {
        if ((argc != 5 && argc != 7 && argc != 9 && argc != 11) || std::string(argv[1]) != "--device" || std::string(argv[3]) != "--seed")
            throw std::invalid_argument("usage: --device cpu|cuda:0 --seed INTEGER [--rollout-length 32|64|128] [--gae-lambda NUMBER] [--financial-features raw|signed-log-v1]");
        ...
        bool has_rollout = false, has_lambda = false, has_financial_features = false;
        ...
        Trainer trainer(std::stoull(argv[4]), torch::Device(argv[2]), rollout_length, gae_lambda, financial_features);
