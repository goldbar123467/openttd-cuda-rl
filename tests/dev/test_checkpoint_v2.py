...
class CheckpointV2Tests(unittest.TestCase):
    ...
    def test_changed_return_or_reuse_config_fails_before_restore(self):
        self.expected["configuration"].update(gamma=.99, gae_lambda=.95, reuse_bootstrap_tensors=False, financial_features="raw")
        (self.root / "checkpoint.json").write_text(json.dumps(self.manifest))
        for key, value in (("gamma", .9), ("gae_lambda", 1.0), ("reuse_bootstrap_tensors", True), ("financial_features", "signed-log-v1")):
