    ...
    def test_relocated_live_source_and_build_use_one_context(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            value, config_path, schema_path, base_source, result_source = make_live_source_fixture(
                ...
                base_set="v2-m15-observation-a",
                base_relative="source",
                result_set="v2-m15-action-a",
                config_filename="action-source.json",
            )
            real_git = validate_m15_action_source.git
            with mock.patch.object(validate_m15_action_source, "git", wraps=real_git) as reader:
                summary = validate_m15_action_source.validate(
                    self.root,
                    config_path,
                    schema_path,
        ...
        with tempfile.TemporaryDirectory() as raw:
            with mock.patch.object(
                ...
                with self.assertRaisesRegex(ArtifactContextError, "missing"):
                    validate_m15_action_source.validate(
                        self.root,
                        artifact_context=ArtifactContext.live(pathlib.Path(raw).resolve()),
                    )
            reader.assert_not_called()

    def test_required_live_inputs_are_the_exact_action_source_closure(self) -> None:
        requirements = validate_m15_action_source.required_live_inputs(self.root)
        self.assertEqual(
            ...
            (
                ...
                ("v2-m15-action-a", "build/openttd", "file"),
            ),
        )
        self.assertEqual({item.consumer for item in requirements}, {"m15-action-source"})
    ...
    def test_schema_hash_drift_fails(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            with self.assertRaisesRegex(validate_m15_action_source.M15ActionSourceError, "schema SHA-256"):
                validate_m15_action_source.validate(self.root, self.write(pathlib.Path(raw), value), self.schema, artifact_context=ArtifactContext.offline())
    ...
    def test_result_tree_drift_fails_live(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            with self.assertRaisesRegex(validate_m15_action_source.M15ActionSourceError, "result[/ ]tree"):
                validate_m15_action_source.validate(self.root, self.write(pathlib.Path(raw), value), self.schema, artifact_context=ArtifactContext.live(base))
    ...
    def test_executable_identity_drift_fails_live(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(ArtifactContextError, "SHA-256 mismatch"):
                validate_m15_action_source.validate(self.root, config_path, schema_path, artifact_context=ArtifactContext.live(base))
