#!/usr/bin/env python3
"""Mutation tests for complete native M18 ship evidence."""
...
class M18ShipEvidenceTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.artifact = pathlib.Path("/home/thecl/.codex/artifacts/openttd-rl/v2-m18-ship-matrix-c")
    ...
    def mutation_fails(self, value: object, pattern: str | None = None) -> None:
        with tempfile.TemporaryDirectory() as raw:
            context = self.assertRaisesRegex(validator.M18EvidenceError, pattern) if pattern else self.assertRaises((validator.M18EvidenceError, ValueError))
            with context:
    ...
    def test_vacuous_income_mutation_fails(self) -> None:
        ...
        target = next(item for item in value["cases"] if item["probe"] == "natural")
