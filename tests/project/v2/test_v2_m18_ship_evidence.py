#!/usr/bin/env python3
"""Mutation and relocated-live tests for complete native M18 ship evidence."""
...
class M18ShipEvidenceTests(unittest.TestCase):
    @classmethod
    ...
    def mutation_fails(self, value: object, pattern: str | None = None) -> None:
        with tempfile.TemporaryDirectory() as raw:
            raised = self.assertRaisesRegex(validator.M18EvidenceError, pattern) if pattern else self.assertRaises((validator.M18EvidenceError, ValueError))
            with raised:
    ...
    def test_repository_evidence_passes(self) -> None:
        ...
        self.assertEqual((summary["cases"], summary["runs"], summary["twin_exact"]), (16, 32, 16))
    ...
    def test_required_live_inputs_are_the_exact_report_closure(self) -> None:
        ...
        self.assertEqual(len(requirements), 32)
        self.assertEqual(len(set(requirements)), 32)
        self.assertEqual(
        ...
        self.assertEqual({item.logical_set for item in requirements}, {"v2-m18-ship-matrix-c"})
    ...
    def test_vacuous_income_mutation_fails(self) -> None:
        value = copy.deepcopy(self.config); next(item for item in value["cases"] if item["probe"] == "natural")["metrics"]["income"] = 0
    ...
    def test_baseline_mutation_fails(self) -> None:
        value = copy.deepcopy(self.config); value["baselines"]["qualification_manifest_sha256"] = "0" * 64
