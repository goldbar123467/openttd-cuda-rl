#!/usr/bin/env python3
"""Offline and mutation tests for M22's frozen follow-up-v2 one-shot runner."""
...
class M22FollowupV2EvaluationSourceTests(unittest.TestCase):
    @classmethod
    ...
    @staticmethod
    def case(case_id: str = "followup-v2-case-00") -> dict[str, object]:
        return {
    ...
    def fake_run(cls, ordinal: int) -> dict[str, object]:
        case = cls.case(f"followup-v2-case-{ordinal:02d}")
        ...
        return {
            "artifact_path": f"cases/{ordinal:02d}-followup-v2-case-{ordinal:02d}", "evaluator": evaluator,
    ...
    @classmethod
    def fake_manifest_run(cls, ordinal: int, case: dict[str, object]) -> dict[str, object]:
        run = cls.fake_run(ordinal)
        public = runner.public_case(case)
        required = case["required_program"]
        run["artifact_path"] = f"cases/{ordinal:02d}-{case['case_id']}"
        run["private_seed"] = case["seed"]
        run["public_case"] = public
        run["required_program"] = required
        run["evaluator"]["action"] = required
        run["evaluator"]["action_index"] = runner.PROGRAM_INDEX[required]
        run["evaluator"]["legal_active_program"] = required
        run["native"]["record"]["case"] = public
        run["native"]["record"]["native_probe"] = case["native_probe"]
        if case["opponent"] != "not-applicable":
            run["native"]["record"]["metrics"]["opponent"] = case["opponent"]
        run["scores"] = runner.case_scores(case, run["evaluator"], run["native"])
        return run
    ...
    def fake_report(cls) -> dict[str, object]:
        ...
        report: dict[str, object] = {
            "acceptance": acceptance, "artifact_root": "/retained/m22-followup-v2", "failure_counts": {
            ...
            "identity": {
                ...
                "immutable_final_v1_evidence_sha256": "f" * 64,
                "immutable_followup_v1_evidence_sha256": "6" * 64,
                "learning_contract_sha256": "0" * 64,
            ...
            "immutable_followup_v1": {
                "cases_attempted": 42, "classified_failures": 0,
                "evidence_path": "config/v2/m22-followup-evaluation-evidence.json",
                "evidence_sha256": "5" * 64, "followup_v2_replaces_followup_v1": False,
                "original_cases_reexecuted": 0, "status": "FAIL",
            },
            "manifest": {"case_count": 42, "id": "m22-independent-followup-v2",
                         "path": "config/v2/m22-followup-v2-manifest.json", "sha256": "6" * 64},
            ...
            "source": {
                ...
                "main_synchronized": True, "repository_commit": "8" * 40,
    ...
    def test_routing_labeled_multimodal_service_is_explicitly_admitted(self) -> None:
        manifest = runner.manifest_validator.load(self.root / runner.MANIFEST)
        runs = [self.fake_manifest_run(index, case) for index, case in enumerate(manifest["cases"])]
        protocol = runner.protocol_record(runs, [case["case_id"] for case in manifest["cases"]])
        acceptance = runner.acceptance(runs, runner.aggregate_statistics(runs), protocol)
        multimodal = [run for run in runs if run["required_program"] == "multimodal-transfer"]
        self.assertEqual(len(multimodal), 2)
        self.assertTrue(all(run["public_case"]["task"] == "routing" for run in multimodal))
        self.assertTrue(acceptance["service_case_contract_exact"])
        self.assertTrue(acceptance["service_every_mode"])
        self.assertTrue(acceptance["overall"])

    def test_routing_labeled_multimodal_service_failure_is_classified_and_rejected(self) -> None:
        manifest = runner.manifest_validator.load(self.root / runner.MANIFEST)
        case = next(case for case in manifest["cases"] if case["required_program"] == "multimodal-transfer")
        run = self.fake_manifest_run(0, case)
        run["native"]["record"]["metrics"]["delivered"] = 0
        run["scores"] = runner.case_scores(case, run["evaluator"], run["native"])
        self.assertIn("native-service", runner.failure_categories(
            case, run["evaluator"], run["native"], run["scores"],
        ))

    def test_acceptance_source_never_uses_task_equals_service_admission(self) -> None:
        source = (self.root / "scripts/v2/run_m22_followup_v2_evaluation.py").read_text(encoding="utf-8")
        acceptance = source[source.index("def acceptance("):source.index("def immutable_final_record(")]
        self.assertNotIn('["public_case"]["task"] == "service"', acceptance)
        self.assertIn('run["required_program"] in SERVICE_PROGRAM_COUNTS', acceptance)
    ...
    def test_independent_validator_recomputes_scores_and_failures(self) -> None:
        ...
        with self.assertRaisesRegex(validator.M22FollowupV2EvidenceError, "case score"):
    ...
    def test_source_inventory_binds_all_followup_and_native_boundaries(self) -> None:
        self.assertEqual(len(runner.SOURCE_PATHS), 29)
        self.assertNotIn("config/v2/m22-followup-v2-manifest.json", runner.SOURCE_PATHS)
        self.assertIn("config/v2/m22-final-evaluation-evidence.json", runner.SOURCE_PATHS)
        self.assertIn("config/v2/m22-followup-evaluation-evidence.json", runner.SOURCE_PATHS)
        self.assertIn("scripts/v2/run_m22_followup_v2_evaluation.py", runner.SOURCE_PATHS)
        self.assertIn("scripts/v2/validate_m22_followup_v2_evaluation.py", runner.SOURCE_PATHS)
        self.assertIn("docs/project/schema/v2-m22-followup-v2-evaluation-evidence.schema.json", runner.SOURCE_PATHS)
        self.assertIn("training/v2/src/m22_evaluator_main.cpp", runner.SOURCE_PATHS)

    def test_protocol_never_retries_or_replaces_either_failed_suite(self) -> None:
        ...
        self.assertEqual(report["immutable_followup_v1"]["original_cases_reexecuted"], 0)
        self.assertFalse(report["immutable_followup_v1"]["followup_v2_replaces_followup_v1"])
