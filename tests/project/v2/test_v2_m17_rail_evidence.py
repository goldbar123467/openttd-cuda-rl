#!/usr/bin/env python3
"""Mutation tests for complete native M17 rail evidence."""
...
class M17RailEvidenceTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.artifact = pathlib.Path("/home/thecl/.codex/artifacts/openttd-rl/v2-m17-rail-matrix-a")
    ...
    def mutation_fails(self, value: object, pattern: str | None = None) -> None:
        with tempfile.TemporaryDirectory() as raw:
            context = self.assertRaisesRegex(validator.M17EvidenceError, pattern) if pattern else self.assertRaises((validator.M17EvidenceError, ValueError))
            with context:
    ...
    def test_vacuous_income_mutation_fails(self) -> None:
        ...
        target = next(item for item in value["cases"] if item["probe"] == "passenger")
