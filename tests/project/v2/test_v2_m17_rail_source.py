#!/usr/bin/env python3
"""Offline and relocated-live tests for the native M17 rail source delta."""
...
import validate_m17_rail_source as validator


class M17RailSourceTests(unittest.TestCase):
    @classmethod
    ...
    def mutation_fails(self, value: object, pattern: str | None = None) -> None:
        with tempfile.TemporaryDirectory() as raw:
            raised = self.assertRaisesRegex(validator.M17SourceError, pattern) if pattern else self.assertRaises(validator.M17SourceError)
            with raised:
    ...
    def test_required_live_inputs_are_the_exact_source_and_build_closure(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                ...
                ("v2-m17-rail-a", "build/openttd", "file"),
                ("v2-m17-rail-a", "build/baseset/opengfx-8.0.tar", "file"),
            ),
        )
        self.assertEqual({item.consumer for item in requirements}, {"m17-rail-source"})
    ...
    def test_relocated_live_source_and_build_pass(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            value, config_path, _, _ = make_live_source_fixture(
                ...
                base_set="v2-m16-cargo-a", result_set="v2-m17-rail-a",
            )
    ...
    def test_source_tree_mutation_fails_live(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            base = pathlib.Path(raw).resolve()
            value, config_path, _, _ = make_live_source_fixture(self.root, base, self.config, base_set="v2-m16-cargo-a", result_set="v2-m17-rail-a")
            value["source"]["tree"] = "0" * 40
            config_path.write_text(json.dumps(value) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(validator.M17SourceError, "tree"):
    ...
    def test_executable_size_mutation_fails_live(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            config_path.write_text(json.dumps(value) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(validator.M17SourceError, "executable identity"):
                validator.validate(self.root, config_path, self.schema, artifact_context=ArtifactContext.live(base))
