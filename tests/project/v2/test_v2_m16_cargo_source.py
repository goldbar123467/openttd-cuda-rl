#!/usr/bin/env python3
"""Mutation tests for the native M16 cargo source delta."""
...
class M16CargoSourceTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.artifact = pathlib.Path("/home/thecl/.codex/artifacts/openttd-rl/v2-m16-cargo-a")
        cls.base = pathlib.Path("/home/thecl/.codex/artifacts/openttd-rl/v2-m15-competence-a/source")
    ...
    def mutation_fails(self, value: object, pattern: str | None = None, *, live: bool = False) -> None:
        with tempfile.TemporaryDirectory() as raw:
            context = self.assertRaisesRegex(validator.M16SourceError, pattern) if pattern else self.assertRaises(validator.M16SourceError)
            with context:
    ...
    def test_repository_source_passes(self) -> None:
        self.assertEqual(validator.validate(self.root)["files"], 8)
