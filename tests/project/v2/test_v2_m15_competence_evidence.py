    ...
    def test_relocated_live_competence_uses_context_before_helper_reads(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            requirements = tuple(
                ...
                for requirement in validate_m15_competence_evidence.required_live_inputs(self.root)
            )
            for requirement in requirements:
                path = base / logical_set / requirement.relative_path
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(b"synthetic competence fixture\n")
            matrix = {
                "outcome": "PASS",
            ...
            with (
                mock.patch.object(
                    validate_m15_competence_evidence,
