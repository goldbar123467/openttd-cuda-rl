#!/usr/bin/env python3
"""Reproduction and mutation tests for the frozen M22 follow-up-v2 manifest."""
...
import validate_m22_followup_v2_manifest as validator
from tests.project.v2 import m22_fixture_support as fixture_support


class M22FollowupV2ManifestTests(unittest.TestCase):
    @classmethod
    ...
    def write(directory: pathlib.Path, value: object) -> pathlib.Path:
        path = directory / "followup-v2-manifest.json"
    ...
    def test_all_seeds_are_derived_unique_and_previously_unseen(self) -> None:
        ...
        self.assertTrue(set(seeds).isdisjoint(builder.PREFLIGHT_SEEDS))
        followup_v1 = validator.load(self.root / builder.IMMUTABLE_FOLLOWUP_V1)
        self.assertTrue(set(seeds).isdisjoint(run["private_seed"] for run in followup_v1["runs"]))
    ...
    def test_followup_v1_replacement_claim_fails_schema(self) -> None:
        value = copy.deepcopy(self.manifest)
        value["access_policy"]["followup_v1_cases"] = "reexecute-original-cases"
        with self.assertRaises(jsonschema.ValidationError):
            jsonschema.Draft202012Validator(self.schema).validate(value)

    def test_aggregate_service_admission_mutation_fails_schema(self) -> None:
        value = copy.deepcopy(self.manifest)
        value["aggregate_contract"]["admission"] = "task-equals-service"
        with self.assertRaises(jsonschema.ValidationError):
            jsonschema.Draft202012Validator(self.schema).validate(value)
    ...
    def test_followup_v1_prerequisite_mutation_fails(self) -> None:
        def mutate(value: dict[str, object]) -> None:
            value["prerequisites"]["immutable_followup_v1_evidence_sha256"] = "0" * 64

        self.run_mutations((fixture_support.MutationCase(
            "followup-v1-prerequisite", mutate, "canonical deterministic build",
        ),))
    ...
    def test_both_failed_suites_and_corrected_runtime_boundaries_remain_separate(self) -> None:
        ...
        self.assertEqual(followup_v1["status"], "FAIL")
        self.assertTrue(all(not run["failures"] for run in followup_v1["runs"]))
