#!/usr/bin/env python3
"""Mutation tests for M15 passenger-service source evidence."""
...
class M15CompetenceSourceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.root = pathlib.Path(__file__).resolve().parents[3]
        cls.config = validate_m15_competence_source.load_json(cls.root / validate_m15_competence_source.CONFIG)
        cls.schema = cls.root / validate_m15_competence_source.SCHEMA

    @staticmethod
    def write(directory: pathlib.Path, value: object) -> pathlib.Path:
        path = directory / "competence-source.json"
    ...
    def test_live_source_and_build_pass(self) -> None:
        summary = validate_m15_competence_source.validate(
            self.root,
    ...
    def test_relocated_live_source_and_build_use_one_context(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            value, config_path, schema_path, base_source, result_source = make_live_source_fixture(
                ...
                base_set="v2-m15-episode-a",
                base_relative="source",
                result_set="v2-m15-competence-a",
                config_filename="competence-source.json",
            )
            real_git = validate_m15_competence_source.git
            with mock.patch.object(validate_m15_competence_source, "git", wraps=real_git) as reader:
                summary = validate_m15_competence_source.validate(
                    self.root,
                    config_path,
                    schema_path,
        ...
        with tempfile.TemporaryDirectory() as raw:
            with mock.patch.object(
                ...
                with self.assertRaisesRegex(ArtifactContextError, "missing"):
                    validate_m15_competence_source.validate(
                        self.root,
                        artifact_context=ArtifactContext.live(pathlib.Path(raw).resolve()),
                    )
            reader.assert_not_called()

    def test_required_live_inputs_are_the_exact_competence_source_closure(self) -> None:
        requirements = validate_m15_competence_source.required_live_inputs(self.root)
        self.assertEqual(
            ...
            (
                ...
                ("v2-m15-competence-a", "build/openttd", "file"),
            ),
        )
        self.assertEqual({item.consumer for item in requirements}, {"m15-competence-source"})
    ...
    def test_schema_hash_drift_fails(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            with self.assertRaisesRegex(validate_m15_competence_source.M15CompetenceSourceError, "schema SHA-256"):
                validate_m15_competence_source.validate(self.root, self.write(pathlib.Path(raw), value), self.schema, artifact_context=ArtifactContext.offline())

    def test_patch_digest_drift_fails(self) -> None:
        value = copy.deepcopy(self.config)
        value["patch"]["sha256"] = "0" * 64
        with tempfile.TemporaryDirectory() as raw:
            with self.assertRaisesRegex(validate_m15_competence_source.M15CompetenceSourceError, "patch SHA-256"):
                validate_m15_competence_source.validate(self.root, self.write(pathlib.Path(raw), value), self.schema, artifact_context=ArtifactContext.offline())
    ...
    def test_result_tree_drift_fails_live(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            with self.assertRaises(validate_m15_competence_source.M15CompetenceSourceError):
                validate_m15_competence_source.validate(self.root, self.write(pathlib.Path(raw), value), self.schema, artifact_context=ArtifactContext.offline())
    ...
    def test_executable_identity_drift_fails_live(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(ArtifactContextError, "SHA-256 mismatch"):
                validate_m15_competence_source.validate(self.root, config_path, schema_path, artifact_context=ArtifactContext.live(base))
