...
continuation; time limits still bootstrap the final physical state. Only the
first four training ledger seeds are used. The reward version and each raw
...
64-step reset recovery also passes exactly on CPU and CUDA; no improved gameplay
claim is established. With a fresh build containing the rollout option, reproduce
...
uses separate reset checkpoints; V2 ONNX export remains unimplemented. Build the
...
512-decision budget. Both controls use exactly the same guide and legality.
