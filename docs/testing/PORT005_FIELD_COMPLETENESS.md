...
| settings revision | `2099` proves no native counter; complete values are `2000`–`2065` |
...
| catchment configuration and exact coverage | settings `2063`–`2065`; fields `6032`, `6050`–`6053`, totals/offsets `6061`, `6068` |
...
| entry, exit, bay and vehicle queue/occupancy | `6112`, `6114`–`6117`, `6036` |
| selected-cargo `GoodsEntry` status/waiting/rating/service | `6200`–`6206`, `6230`–`6232` |
| packet chain head/order | `6209`, packet owner/order `9021`–`9023` |
| load/unload state | `6033`, `6034`, `6036`, `6232`, vehicle `7036`, `7183`–`7186`; truck area dimensions and visit mask `6039`–`6041` |
| station caches and invalidation flags/triggers | `6032`, `6038`, `6044`–`6059`, `6207`, `6208`, `6210`–`6214`, `6230`–`6232`; GoodsEntry presence/packet/flow totals and nested offsets are `6060`–`6080`; all continuation caches remain authoritative |
...
| speed/progress/acceleration/controller | `7020`–`7023`, `7070`–`7076`, caches `7090`–`7101`, movement/order flags `7110`; display-only last speed/effect are diagnostics `7111`, `7112` |
...
| Required review item | Field IDs / source-backed disposition |
|---|---|
| order pool occupancy/stable ID | `8099` proves no global pool; OrderList allocation is `8000`–`8004`; Order identity is `(OrderListID, ordinal)` and total count is `8039` |
...
CargoPayment `12300`–`12315`, and Subsidy `12400`–`12417`. LinkGraph pool
occupancy `12500`–`12503`, `12531` is asserted empty; detailed graph/node/edge,
schedule and job families `12505`–`12530` are source-backed unreachable under
manual cargo distribution and must be promoted to full fields if that setting
or command scope changes.
...
| `Station`, `BaseStation`, `GoodsEntry` and `FlowStat` | Truck area includes tile/width/height; visit mask and neutral-industry/name state are stored. `GoodsEntryData` has independent pointer presence. Flows are nested origin maps whose shares are cumulative-key/via maps. | Station entry/loading/rating and random next-hop selection observe these members. Presence, exact packet order, origins, unrestricted totals, shares and both offset levels are full fields. |
| `Vehicle`, `GroundVehicle`, `RoadVehicle` | `BaseVehicle::type`, `gv_flags`, `compatible_roadtypes`, controller counters and path vector are distinct native members. `GroundVehicleCache::last_speed` and `VehicleCache::cached_vis_effect` have display consumers only. | Type/flags/compatibility/path alter command dispatch, order processing and movement and are full. Last display speed and visual effect are diagnostics, not continuation state. |
...
| `LinkGraph`, `LinkGraphSchedule`, `LinkGraphJob` | A complete reachable model would require every BaseNode/BaseEdge member plus schedule and running-job identity/order. | Manual cargo distribution prevents graph/job allocation in the frozen fixture. Pool zero state is full; all detailed families are explicit unreachable proofs rather than a partial authoritative claim. |
