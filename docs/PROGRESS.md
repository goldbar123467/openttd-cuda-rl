...
drive tuning. The V2 training-budget diagnostic failed its criteria; longer
rollouts are undergoing correctness checks before a matched-budget experiment. Frozen release
...
weights differ between devices. Native rejection tests are running before training.
