...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None, *, base_source: pathlib.Path | None = None, artifact_root: pathlib.Path | None = None) -> M15EpisodeSourceSummary:
    root = root.resolve()
    ...
    if artifact_root is not None:
        ...
        require(str(artifact_root) == config["build"]["artifact_root"], "M15 episode build artifact root drifted")
    ...
    return M15EpisodeSourceSummary(len(touched), config["result"]["tree"], base_source is not None, artifact_root is not None)
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except (M15EpisodeSourceError, OSError) as exc:
