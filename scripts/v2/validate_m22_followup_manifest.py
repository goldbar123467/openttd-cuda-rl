#!/usr/bin/env python3
"""Independently validate the frozen, pre-access M22 follow-up protocol."""
...
class M22FollowupManifestError(ValueError):
    """The M22 follow-up protocol is incomplete, non-independent, or drifted."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise M22FollowupManifestError(message)
...
def load(path: pathlib.Path) -> dict[str, Any]:
    try:
        ...
        raise M22FollowupManifestError(f"cannot load JSON {path}: {exc}") from exc
...
def schema_validate(value: dict[str, Any], schema: dict[str, Any]) -> None:
    try:
        ...
        raise M22FollowupManifestError(f"follow-up manifest schema failed at {where}: {exc.message}") from exc
...
def validate_value(root: pathlib.Path, manifest: dict[str, Any], manifest_bytes: bytes) -> dict[str, Any]:
    ...
    require(manifest == expected and manifest_bytes == builder.canonical_bytes(expected),
            "follow-up manifest is not the canonical deterministic build")
    ...
    require(runtime["status"] == "PASS" and runtime["boundaries"]["immutable_final_v1"]["status"] == "FAIL" and
            ...
            }, "corrected runtime pre-follow-up boundary drifted")
    selected = qualification["finalized_selection"]
    require(selected["finalized"] and not selected["final_manifest_accessed"] and
            selected["checkpoint_id"] == immutable["identity"]["checkpoint_id"],
            "selected checkpoint changed across final-v1 and follow-up freeze")

    cases, base_cases = manifest["cases"], base["cases"]
    require(len(cases) == len(base_cases) == 42 and len({case["case_id"] for case in cases}) == 42,
            "follow-up case identity closure drifted")
    require({case["case_id"] for case in cases}.isdisjoint(case["case_id"] for case in base_cases) and
            all(case["case_id"] == f"followup-{old['case_id']}" for case, old in zip(cases, base_cases, strict=True)),
            "follow-up case IDs do not preserve distinct deterministic lineage")
    require([comparable_case(case) for case in cases] == [comparable_case(case) for case in base_cases],
            "follow-up changed more than case identity, seed, or corrected G20 dimensions")
    ...
    require(seeds == derived and len(set(seeds)) == 42 and not set(seeds).intersection(excluded),
            "follow-up seed derivation, uniqueness, or unseen boundary drifted")
    require(Counter(case["required_program"] for case in cases) ==
            Counter(case["required_program"] for case in base_cases),
            "follow-up program distribution drifted from the original independent suite")
    ...
    for case in cases:
        require(case["required_program"] in final.PROGRAM_INDEX and
                ...
                f"follow-up public capability/program gate drifted: {case['case_id']}")
    ...
    require(len(competition) == 6 and all((case["map_width"], case["map_height"]) == (128, 128)
            ...
            "follow-up competition contract is not two 128x128 cases per qualified opponent")
    require({case["transport_mode"] for case in cases} == learning.MODES and
            ...
            "follow-up mode, climate, size, or opponent coverage drifted")
    return {
...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise M22FollowupManifestError(f"cannot load JSON {manifest_path}: {exc}") from exc
...
def main() -> int:
    ...
    except (M22FollowupManifestError, builder.M22FollowupManifestBuildError, OSError, KeyError, TypeError,
            ValueError) as exc:
        print(f"V2_M22_FOLLOWUP_MANIFEST=FAIL {exc}", file=sys.stderr)
        return 1
    print(f"V2_M22_FOLLOWUP_MANIFEST=PASS cases={result['cases']} programs={result['programs']} "
