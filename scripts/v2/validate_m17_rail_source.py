#!/usr/bin/env python3
"""Validate the retained native M17 rail source delta, patch, and executable."""
...
CONFIG = pathlib.Path("config/v2/m17-rail-source.json")
SCHEMA = pathlib.Path("docs/project/schema/v2-m17-rail-source.schema.json")
TOUCHED = ["src/CMakeLists.txt", "src/openttd.cpp", "src/rl_v2_rail.cpp", "src/rl_v2_rail.h"]
BASE_LOGICAL_SET = "v2-m16-cargo-a"
RESULT_LOGICAL_SET = "v2-m17-rail-a"
LIVE_CONSUMER = "m17-rail-source"


class M17SourceError(ValueError):
    """M17 native source evidence is inconsistent."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise M17SourceError(message)
...
def invoke_git(*arguments: str, repository: pathlib.Path | None = None):
    try:
        ...
        raise M17SourceError(f"git {' '.join(arguments)} failed: {exc}") from exc
...
def _recorded_result_set(config: dict[str, Any]) -> str:
    ...
    require(isinstance(recorded, str) and recorded.startswith("/") and not recorded.startswith("//") and str(path) == recorded and all(part not in {"", ".", ".."} for part in path.parts[1:]), "recorded retained artifact is not an absolute normalized POSIX path")
...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None,
    ...
    try:
        ...
        raise M17SourceError(f"source schema failed at {where}: {exc.message}") from exc
    ...
    for token in (
        "RunRlV2RailQualification", "CMD_BUILD_SINGLE_RAIL", "CMD_BUILD_RAIL_STATION", "CMD_REMOVE_FROM_RAIL_STATION",
        "CMD_BUILD_SINGLE_SIGNAL", "TryReserveTrack", "CMD_BUILD_VEHICLE", "CMD_MOVE_RAIL_VEHICLE", "CMD_CHANGE_TIMETABLE",
        "CMD_SET_AUTOREPLACE", "CMD_AUTOREPLACE_VEHICLE", "StateGameLoop", "RlV2SetQualificationAcceptance",
    ):
    ...
    if context.is_live:
        ...
        with tempfile.TemporaryDirectory(prefix="openttd-rl-v2-m17-source-") as raw:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"V2_M17_RAIL_SOURCE=PASS files={summary['files']} tree={summary['tree']} live={str(summary['live']).lower()}")
        return 0
    except (M17SourceError, SourceContextError, ArtifactContextError, OSError, json.JSONDecodeError) as exc:
        print(f"V2_M17_RAIL_SOURCE=FAIL {exc}")
        return 1
