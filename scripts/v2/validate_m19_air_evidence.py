...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None,
    ...
    require(evidence["baselines"] == matrix.baseline_evidence(root), "opponent baseline evidence drifted")
...
def main() -> int:
    ...
    try:
        ...
        print(f"V2_M19_AIR_EVIDENCE=PASS cases={summary['cases']} runs={summary['runs']} twin_exact={summary['twin_exact']}")
        return 0
    except (M19EvidenceError, matrix.M19MatrixError, OSError, json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
