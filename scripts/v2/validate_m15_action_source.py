...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None, *, artifact_context: ArtifactContext | None = None) -> M15ActionSourceSummary:
    ...
    if context.is_live:
        ...
        with tempfile.TemporaryDirectory(prefix="openttd-rl-v2-m15-action-source-") as raw:
            ...
            require(cloned.returncode == 0, f"cannot clone action base source: {clone_detail}")
            ...
            require(checked.returncode == 0 and not re.search(r"\b(?:offset|fuzz|warning)\b", check_output, re.I), "M15 action patch does not apply exactly")
            ...
            require(applied.returncode == 0, f"cannot apply M15 action patch: {apply_detail}")
