#!/usr/bin/env python3
"""Validate the retained native M19 aircraft source delta, patch, and executable."""
...
CONFIG = pathlib.Path("config/v2/m19-air-source.json")
SCHEMA = pathlib.Path("docs/project/schema/v2-m19-air-source.schema.json")
TOUCHED = ["src/CMakeLists.txt", "src/openttd.cpp", "src/rl_v2_air.cpp", "src/rl_v2_air.h"]
BASE_LOGICAL_SET = "v2-m18-ship-a"
RESULT_LOGICAL_SET = "v2-m19-air-a"
LIVE_CONSUMER = "m19-air-source"


class M19SourceError(ValueError):
    """M19 native source evidence is inconsistent."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise M19SourceError(message)
...
def invoke_git(*arguments: str, repository: pathlib.Path | None = None):
    try:
        ...
        raise M19SourceError(f"git {' '.join(arguments)} failed: {exc}") from exc
...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None,
    ...
    try:
        ...
        raise M19SourceError(f"source schema failed at {where}: {exc.message}") from exc
    ...
    for token in (
        "RunRlV2AirQualification", "CMD_BUILD_AIRPORT", "CMD_OPEN_CLOSE_AIRPORT", "airport.blocks", "AirportClosed",
        "CMD_BUILD_VEHICLE", "CMD_REFIT_VEHICLE", "CMD_CLONE_ORDER", "CMD_CHANGE_TIMETABLE", "CMD_CHANGE_SERVICE_INT",
        "CMD_SET_AUTOREPLACE", "CMD_AUTOREPLACE_VEHICLE", "SaveOrLoad", "StateGameLoop", "CMD_BUILD_DOCK",
        "CMD_BUILD_CANAL", "CMD_BUILD_ROAD_STOP", "RlV2SetQualificationAcceptance", "RunMultimodal", "RunRouter",
    ):
    ...
    if context.is_live:
        ...
        with tempfile.TemporaryDirectory(prefix="openttd-rl-v2-m19-source-") as raw:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"V2_M19_AIR_SOURCE=PASS files={summary['files']} tree={summary['tree']} live={str(summary['live']).lower()}")
        return 0
    except (M19SourceError, SourceContextError, ArtifactContextError, OSError, json.JSONDecodeError) as exc:
        print(f"V2_M19_AIR_SOURCE=FAIL {exc}")
        return 1
