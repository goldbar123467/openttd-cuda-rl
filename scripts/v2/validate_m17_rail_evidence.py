...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None, *, artifact_root: pathlib.Path | None = None) -> dict[str, Any]:
    ...
    return {"cases": len(expected_cases), "runs": evidence["aggregate"]["native_runs"], "twin_exact": evidence["aggregate"]["twin_exact_cases"]}
...
def main() -> int:
    ...
    try:
        ...
        print(f"V2_M17_RAIL_EVIDENCE=PASS cases={summary['cases']} runs={summary['runs']} twin_exact={summary['twin_exact']}")
        return 0
    except (M17EvidenceError, matrix.M17MatrixError, OSError, json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
