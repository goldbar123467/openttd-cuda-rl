...
def load(path: pathlib.Path) -> dict[str, Any]:
    return evaluation_common.load_json_object(path, error_type=M22FollowupEvidenceError)


def schema_validate(value: dict[str, Any], schema: dict[str, Any], label: str) -> None:
    evaluation_common.validate_schema(value, schema, label, error_type=M22FollowupEvidenceError)
...
def validate_value(
    ...
    evaluation_common.validate_report_digest(
        report, mechanics=runner, suite_label="M22 follow-up", require=require,
    )
    ...
    result = evaluation_common.validate_aggregate_records(
        report, cases, mechanics=runner, suite_label="M22 follow-up",
        live=context.is_live, require=require,
    )
    require(len(cases) == 42 and report["status"] == "FAIL" and result["failures"] == 0,
