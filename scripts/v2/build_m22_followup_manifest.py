#!/usr/bin/env python3
"""Reproduce the frozen, unseen M22 independent follow-up manifest."""
...
MANIFEST = pathlib.Path("config/v2/m22-followup-manifest.json")
SCHEMA = pathlib.Path("docs/project/schema/v2-m22-followup-manifest.schema.json")
...
DOMAIN = "openttd-rl-v2-m22-independent-followup-v1"
ORDINAL_START = 2000
...
class M22FollowupManifestBuildError(ValueError):
    """The independent follow-up manifest cannot be reproduced safely."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise M22FollowupManifestBuildError(message)
...
def load(path: pathlib.Path) -> dict[str, Any]:
    try:
        ...
        raise M22FollowupManifestBuildError(f"cannot load JSON {path}: {exc}") from exc
...
def seed_source_records(root: pathlib.Path) -> tuple[list[dict[str, str]], set[int]]:
    ...
    seeds: set[int] = set(EXTERNAL_DIAGNOSTIC_SEEDS)
...
def build(root: pathlib.Path) -> dict[str, Any]:
    ...
    require(len(cases) == len(derived) == 42 and len(set(derived)) == 42,
            "follow-up seed inventory is not exactly 42 unique values")
    require(not excluded.intersection(derived), "follow-up seed overlaps a prior M22 or diagnostic seed")
    for index, case in enumerate(cases):
        case["case_id"] = f"followup-{case['case_id']}"
        case["seed"] = derived[index]
        if case["source_gate"] == "G20":
            case["map_width"] = 128
            case["map_height"] = 128
    return {
        "$schema": "../../docs/project/schema/v2-m22-followup-manifest.schema.json",
        ...
        "access_policy": {
            ...
            "followup_runner": "fresh-process-read-only-once-after-source-freeze",
        ...
        "cases": cases,
        "manifest_id": "m22-independent-followup-v1",
        "prerequisites": {
        ...
        "purpose": "independent-post-diagnosis-confirmation-not-retry-or-replacement",
        "schema_sha256": sha256(root / SCHEMA),
        "schema_version": "openttd-rl-v2-m22-followup-manifest-1",
        "seed_derivation": {
...
def main() -> int:
    ...
    try:
        ...
        require(path.is_file() and not path.is_symlink(), f"frozen follow-up manifest is absent: {path}")
        require(path.read_bytes() == canonical_bytes(value), "frozen follow-up manifest is not reproducible")
    except (M22FollowupManifestBuildError, OSError, KeyError, TypeError, ValueError) as exc:
        print(f"V2_M22_FOLLOWUP_MANIFEST_BUILD=FAIL {exc}", file=sys.stderr)
        return 1
    print("V2_M22_FOLLOWUP_MANIFEST_BUILD=PASS cases=42 unseen=true competition=128x128")
    return 0
