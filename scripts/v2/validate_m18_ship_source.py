...
BASE_LOGICAL_SET = "v2-m17-rail-a"
RESULT_LOGICAL_SET = "v2-m18-ship-a"
LIVE_CONSUMER = "m18-ship-source"
...
def invoke_git(*arguments: str, repository: pathlib.Path | None = None):
    try:
        ...
        raise M18SourceError(f"git {' '.join(arguments)} failed: {exc}") from exc
...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None,
    ...
    if context.is_live:
        ...
        with tempfile.TemporaryDirectory(prefix="openttd-rl-v2-m18-source-") as raw:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except (M18SourceError, SourceContextError, ArtifactContextError, OSError, json.JSONDecodeError) as exc:
