#!/usr/bin/env python3
"""Validate the exact M15 bounded-observation source delta and retained build."""
...
SCHEMA = pathlib.Path("docs/project/schema/v2-m15-observation-source.schema.json")
BASE_LOGICAL_SET = "v2-m15-native-a"
RESULT_LOGICAL_SET = "v2-m15-observation-a"
LIVE_CONSUMER = "m15-observation-source"
EXPECTED_FILES = [
    "src/CMakeLists.txt", "src/openttd.cpp", "src/rl_v2_environment.cpp", "src/rl_v2_environment.h",
    "src/rl_v2_observation.cpp", "src/rl_v2_observation.h",
]
FORBIDDEN_V1_FILES = {
...
class M15ObservationSourceError(ValueError):
    """The M15 bounded-observation source or build evidence is inconsistent."""


@dataclass(frozen=True)
class M15ObservationSourceSummary:
    files: int
    result_tree: str
    live_source: bool
    live_build: bool
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise M15ObservationSourceError(message)
...
def load_json(path: pathlib.Path) -> dict[str, Any]:
    try:
        ...
        raise M15ObservationSourceError(f"cannot load JSON {path}: {exc}") from exc
...
def _recorded_result_set(config: dict[str, Any]) -> str:
    ...
    require(
        ...
        "M15 observation recorded build root is not an absolute normalized POSIX path",
    )
    require(path.name == RESULT_LOGICAL_SET, "M15 observation logical artifact set drifted")
    return path.name
...
def validate(root: pathlib.Path, config_path: pathlib.Path | None = None, schema_path: pathlib.Path | None = None, *, artifact_context: ArtifactContext | None = None) -> M15ObservationSourceSummary:
    ...
    try:
        ...
        raise M15ObservationSourceError(f"M15 observation source schema failed at {location}: {exc.message}") from exc
    require(config["schema_sha256"] == sha256_file(schema_path), "M15 observation source schema SHA-256 mismatch")
    require(config["base"]["native_source_evidence_sha256"] == sha256_file(root / "config/v2/m15-native-source.json"), "M15 native source evidence identity drifted")
    patch = root / config["patch"]["path"]
    require(patch.is_file() and not patch.is_symlink(), "M15 observation patch is missing or a symlink")
    require(sha256_file(patch) == config["patch"]["sha256"], "M15 observation patch SHA-256 mismatch")
    patch_text = patch.read_text(encoding="utf-8")
    touched = re.findall(r"^diff --git a/(\S+) b/\S+$", patch_text, re.MULTILINE)
    require(touched == EXPECTED_FILES, f"M15 observation patch file scope drifted: {touched}")
    require(not (set(touched) & FORBIDDEN_V1_FILES), "M15 observation patch modifies a frozen V1 RL source file")
    for token in ("OBSERVATION_BYTES = 2182927", "SPATIAL_CHANNELS = 32", "WriteRlV2Observation", "-W"):
        require(token in patch_text, f"M15 observation patch lost required token: {token}")
    ...
    if context.is_live:
        ...
        require(git(base_source, "status", "--porcelain") == "", "M15 observation base source is dirty")
        require(git(base_source, "rev-parse", "HEAD") == config["base"]["commit"], "M15 observation base commit drifted")
        ...
        require(git(result_source, "rev-parse", "HEAD^{tree}") == config["result"]["tree"], "M15 observation retained tree drifted")
        require(executable.is_file() and not executable.is_symlink(), "M15 observation executable is missing or a symlink")
        require(executable.stat().st_size == config["build"]["executable"]["size"], "M15 observation executable size drifted")
        require(sha256_file(executable) == config["build"]["executable"]["sha256"], "M15 observation executable SHA-256 drifted")
    return M15ObservationSourceSummary(len(touched), config["result"]["tree"], context.is_live, context.is_live)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"V2_M15_OBSERVATION_SOURCE=PASS files={summary.files} result_tree={summary.result_tree} live_source={str(summary.live_source).lower()} live_build={str(summary.live_build).lower()}")
        return 0
    except (M15ObservationSourceError, ArtifactContextError, OSError) as exc:
        print(f"V2_M15_OBSERVATION_SOURCE=FAIL {exc}")
        return 1
