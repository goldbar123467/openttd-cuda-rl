...
def load(path: pathlib.Path) -> dict[str, Any]:
    try:
        ...
        raise M22FinalEvidenceError(f"cannot load JSON {path}: {exc}") from exc
...
def validate_live_native(run: dict[str, Any], case_root: pathlib.Path, identity: dict[str, Any]) -> None:
    ...
    require(runner.artifact_inventory(root) == run["native"]["artifact_inventory"],
            f"M22 native artifact inventory drifted: {run['public_case']['case_id']}")
    ...
    require(record["case"] == run["public_case"] and
            ...
            f"M22 native public/source identity drifted: {run['public_case']['case_id']}")
    for path_key, digest_key in (("manifest_path", "manifest_sha256"), ("report_path", "report_sha256"),
        ...
        require(path.is_file() and not path.is_symlink() and runner.sha256(path) == record[digest_key],
                f"M22 native artifact identity drifted: {run['public_case']['case_id']}/{path_key}")
...
def validate_run(
    ...
    if native_result["status"] == "PASS":
        ...
        require(native_result["failure_category"] == "native-execution" and native_result["record"] is None,
                f"M22 native failure record drifted: {case['case_id']}")
    ...
    if artifact_root is not None:
        ...
        require(case_root.is_dir() and not case_root.is_symlink(), f"M22 case artifact root is absent: {case['case_id']}")
...
def validate_value(
    ...
    require(claimed == runner.sha256_bytes(runner.canonical_bytes(unsigned)), "M22 final report digest drifted")
    ...
    require(report["protocol"] == expected_protocol, "M22 final protocol accounting drifted")
    expected_statistics = runner.aggregate_statistics(report["runs"])
    require(report["statistics"] == expected_statistics, "M22 final statistics drifted")
    expected_acceptance = runner.acceptance(report["runs"], expected_statistics, expected_protocol)
    require(report["acceptance"] == expected_acceptance, "M22 final acceptance recomputation drifted")
