...
def compare_completed(root):
    ...
    if rollout not in (32, 64) or full["episode_horizon"] != 128:
        raise ValueError("V2 comparison requires 32/64-step rollouts and 128-decision episodes")
...
def run(args):
    ...
    report = {"kind": "v2-native-reset-resume-verification", "status": "running", "source": source_identity(),
              ...
              "training_map_count": args.training_map_count,
