...
def run(args):
    ...
    financial_features = config.get("financial_features", "raw")
    ...
    report = {"kind": "v2-native-checkpoint-boundary-tests", "status": "running", "source": source_identity(),
        ...
        "trainer_sha256": config["trainer_sha256"], "device": config["device"], "rollout_steps": rollout, "gae_lambda": gae_lambda,
        "financial_features": financial_features, "cases": [],
    ...
    try:
        ...
        if "financial_features" in config:
            cases.append(("wrong-financial-features", [restore], "checkpoint configuration", config["run_seed"]))
        for name, requests, reason, seed in cases:
            ...
            selected_financial = ("signed-log-v1" if financial_features == "raw" else "raw") if name == "wrong-financial-features" else financial_features
            if selected_financial != "raw":
                command.extend(["--financial-features", selected_financial])
