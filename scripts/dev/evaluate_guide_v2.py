#!/usr/bin/env python3
"""Measure uniform and scripted controls under the identical public planner mask."""
import argparse
import hashlib
import json
...
def run(args):
    ...
    record = {"kind": "native-v2-guided-baseline", "status": "running",
              "source": source_identity(), "guidance": GUIDANCE, "controller": args.controller,
    ...
    try:
        ...
        guide = PublicPlanGuide(initial, root / "worker")
        transitions = []
        with (root / "choices.jsonl").open("x") as choices:
            for decision in range(args.decisions):
                ...
                if args.controller == "uniform":
                    ...
                    # The deterministic control builds immediately, then pays
                    # debt using only a repayment already in the same mask.
                    if guide.families[candidates[row]["family_index"]] == "WAIT":
...
if __name__ == "__main__":
    ...
    parser.add_argument("--controller", choices=("uniform", "scripted"), required=True)
