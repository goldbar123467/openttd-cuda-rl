...
class PolicyClient:
    def __init__(self, executable, output, device, seed, mode=None, weights=None, rollout_length=None):
...
def run(args):
    ...
    record = {"kind": "native-v2-neural-live-inference-smoke", "status": "running", "source": source_identity(),
              ...
              "split": args.split, "map_seed": args.map_seed, "final_evaluation_accessed": False,
    ...
    try:
        ...
        game = LiveV2(args.openttd, root / "worker", decisions=args.decisions, split=args.split, seed=args.map_seed)
