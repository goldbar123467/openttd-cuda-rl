...
def read_run(path):
    ...
    if record["rollout_steps"] not in (32, 64, 128) or len(rows) != record["requested_updates"] * record["rollout_steps"]:
...
def run(args):
    ...
    for key, default in (("gamma", .99), ("gae_lambda", .95), ("reuse_bootstrap_tensors", False), ("financial_features", "raw")):
