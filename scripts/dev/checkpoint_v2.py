...
def validate_interval(interval, horizon, rollout=32):
    if rollout not in (32, 64):
        raise ValueError("V2 rollout length must be 32 or 64")
...
def reset_signature(engine, output, configuration, next_episode):
    ...
    try:
        ...
        if configuration["guidance"] == GUIDANCE:
            guide = PublicPlanGuide(observation, output)
