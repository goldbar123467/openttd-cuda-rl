...
Frozen-policy borrowing recovery averages 1,230.67 passengers and 4,009 operating
profit over six sampled development games, versus uniform's 1,193.83 and 3,682.17.
Cash excluding financing remains negative (-5,908.50). The paired profit advantage
is +326.83 with an approximate 95% interval of [-18.82, 672.48], conditional on
three action seeds and two fixed maps. This does not establish a reliable advantage
across training seeds or unseen maps. Retraining's failure remains failed.

![Why action representation and the public guide matter](docs/images/ppo-action-representation-2026-09-24.png)

![Established foundations, current blockers and full-game goals](docs/images/ppo-project-roadmap-2026-09-24.png)

The illustrations and their generation prompts are documented in
[the image notes](docs/images/README.md).
...
research domains and 145 command occurrences. M22/G22 generalist learning now
passes; M23 release remains incomplete. The accepted M22 foundation freezes a
17-program learning contract, a 42-case final-only evaluation manifest, and a
reproducible 32-entry native training/development corpus. The current execution
checkpoint adds a production recurrent PPO trainer, a development-retention
campaign, and atomic exact-recovery checkpoints, all exercised on CPU and CUDA.
The declared update-16 recovery fork now matches exactly for both learned
architectures in six isolated fresh processes. A fail-closed matched-campaign
runner and evidence validator now cover the six architecture-by-seed development
runs and their public heuristic, seeded-random, and wait-only baselines. The
first clean run completed all six 48-update processes, but the set was rejected because
...
protocol was then source-frozen around 42 new unseen seeds and the corrected
competition map contract. Its sole execution retained all 42 cases in order,
with 42 fresh evaluator processes, 42 fresh native processes, zero retries,
zero replacements, and zero classified failures. Every learned action, native
case, opponent, broad probe, climate, map size, and confidence requirement
passed. The frozen aggregate nevertheless remains `FAIL`: its service predicate
selected only `task=service`, omitting both positive routing-labeled multimodal
service cases, so `service_every_mode` alone is false. This result is retained
without relabeling or rerunning. V2-DEF-0006 records that aggregate defect. A
second independent follow-up-v2 protocol was separately frozen before access. It
uses 42 further unseen seeds, binds both failed suites as immutable evidence and
seed exclusions, and admits service by an exact required-program inventory so
the two routing-labeled multimodal-transfer cases cannot disappear from
aggregation. Its sole execution read the manifest once and retained all 42 cases
in order with 42 fresh optimizer-free evaluator processes, 42 fresh network-
unshared native processes, zero retries, zero replacements, and zero classified
failures. Every program, mode, climate, size, opponent, broad probe, and G15-G21
native-retention predicate passes. Learned mean return is `1.54319060396`; the
lower paired 95-percent confidence bounds over seeded-random-legal and wait-only
...
results, not retained G23 visible-playback, reproduction, or release evidence. A second
foundation now exact-copies both seven-file checkpoints and creates both frozen
six-file deployment layouts. Two independent package roots are byte-identical;
their development package IDs are `50060fd871d3c737b41bb4523748fbaac5047fed106e9ce0a9d1b36c7637f955`
and `d280683090b65eeea8e6cba1ab6ece3ca561a1b1d1708840f8616855ff44ac5a`.
The Python validator and inference-only C++ loader accept both, while all 28
frozen package, graph, tensor, recurrence, corruption, and batch mutations fail
before inference. The same loader and shared equivalence implementation now
...
· [`M22 immutable failed follow-up-v1 evidence`](config/v2/m22-followup-evaluation-evidence.json)
· [`M22 frozen independent follow-up-v2 manifest`](config/v2/m22-followup-v2-manifest.json)
· [`M22 follow-up-v2 manifest builder`](scripts/v2/build_m22_followup_v2_manifest.py)
· [`M22 follow-up-v2 manifest validator`](scripts/v2/validate_m22_followup_v2_manifest.py)
· [`M22 follow-up-v2 one-shot runner`](scripts/v2/run_m22_followup_v2_evaluation.py)
...
· [`M23 package builder and validator library`](scripts/v2/m23_package.py)
· [`M23 two-architecture package builder`](scripts/v2/build_m23_packages.py)
...
The path-neutral package builder now exact-copies both frozen seven-file M22
checkpoints without rewriting any byte, creates each exact six-file deployment
inventory, rechecks the embedded ONNX graph and independent golden definitions,
and computes the V1-compatible canonical manifest content address. Two
independent roots built from the independently exported graphs and golden
binaries match byte-for-byte. The monolithic and specialist development package
IDs are respectively
`50060fd871d3c737b41bb4523748fbaac5047fed106e9ce0a9d1b36c7637f955`
and `d280683090b65eeea8e6cba1ab6ece3ca561a1b1d1708840f8616855ff44ac5a`;
the common package-build report SHA-256 is
`d9e533a0c948ec7096017597fb1f4bca6f184fe8da8fe733d39ecd09ee2dfa90`.
The independent Python validator and inference-only C++ package loader accept
both packages. The C++ runtime closure contains ONNX Runtime, OpenSSL Crypto,
and standard system libraries only. Every one of the 28 frozen rejection labels
is implemented and passes: 24 package/manifest/graph/file mutations are rejected
by both validators, and four nonfinite/mask/batch request mutations are rejected
by the runtime. The mutation report SHA-256 is
`33b6311b526bdb7fbadd94b982eb5032576c0769418562cd1ee8a1fc5836c906`.
Fifteen additional package tests cover canonical identity, strict JSON, complete
inventories, exact graph semantics, golden recurrence, evaluation links, host
path leakage, and the C++ load/request boundary.

The next source-integrated foundation applies exactly to accepted M22 source
tree `f8985045f9ba14bad1e46a81cb58fdbb8037f277` and produces tree
`df143cb3ad2ada1023ce63c98ec90f941e48afd8`. It preserves the V1 `-A`
playback entrypoint and the M03 `-B read_fd:write_fd` route while admitting an
absolute M23 JSON configuration through `openttd -B`. The joint V1/V2 GUI build
passed all 98 upstream CTests and links ONNX Runtime 1.28.0 and OpenSSL Crypto
without LibTorch, CUDA, or Python. In a bubblewrap network namespace, OpenTTD
validated both packages and reproduced all 48 cases and 580 rows with exact
actions and the same `1.33514e-05` maximum absolute error. The 48 native, 48
standalone, and 48 source-integrated records total the frozen 144; after removing
only the runtime label, the standalone and in-game reports are identical. The
in-game report SHA-256 is
`23f784c7c29580f4faeceda34ba75bef539bb7c46b1664d87e709ba3949b9532`,
and the independently revalidated foundation report SHA-256 is
`7f3794d75a5b736334fd3a34196a1cc9347a952414dcb64cd6f6898a8af1d67a`.
Fourteen new tests cover patch scope/application, V1/M03 compatibility, the
...
- The M22 checkpoint freezes 17 bounded programs, seven G15-G21 curriculum
  ...
  protocol count, and acceptance result. The source-frozen runner then opened
  this manifest exactly once and retained all 42 cases in order: 42 evaluator
  attempts/processes, 42 native dispatches/processes, no retry, no replacement,
  and no post-result selection. All 42 learned actions equal their required
  programs; every native case is `PASS`; all failure counts are zero; every
  opponent, broad probe, climate, map size, program, and G15-G21 native-retention
  check passes. Learned mean return is `1.54411`; the lower paired 95-percent
  confidence bounds over seeded-random-legal and wait-only are `0.79532` and
  `1.42295`. The retained report still truthfully evaluates to `FAIL` because
  `service_every_mode` alone is false. Its frozen acceptance function collected
  service modes only from cases with `task=service`, while both
  `multimodal-transfer` cases are correctly labeled `task=routing` and
  `transport_mode=multimodal`. Those two native cases delivered 17 and 23 units,
  earned 360 and 488 income, and otherwise passed, but were absent from that
  aggregate set. Independent offline and live validation reports
  `V2_M22_FOLLOWUP_EVIDENCE=FAIL cases=42 failures=0`; evidence report SHA-256 is
  `16faac5a8f83a185f619ca907017ebd1e22900ad90b19c31f95eccf2d8125d0e`
  and the retained evidence file SHA-256 is
  `8288143d77fdb6526efda1e9e60b108ecd6e5eccc4d672ee12b336e759bf5de5`.
  V2-DEF-0005 is closed by the 42/42 corrected native executions. Follow-up-v1
  is immutable failed evidence and will not be rerun, replaced, or relabeled.
  A distinct follow-up-v2 manifest was frozen before access. It reproduces the
  original 42-case coverage shape, retains the corrected six 128-by-128 G20
  worlds, assigns case IDs prefixed `followup-v2-`, and derives 42 unique seeds
  from domain `openttd-rl-v2-m22-independent-followup-v2` at ordinals 3000–3041.
  Its seed-exclusion inventory content-addresses 13 repository records, including
  both earlier manifests and evidence reports, plus the six nonaccepting
  diagnostics and both evaluator-preflight seeds. Manifest SHA-256 is
  `1ba5ff295520d30c88a4d2282804992eedab8cb8f8c1174058e978da63725f4e`.
  The aggregate contract freezes nine service programs and exact counts, maps
  each to road, rail, water, air, or multimodal, requires positive delivery and
  income, and selects them by required-program membership independently of task
  labels. Both `multimodal-transfer` cases are explicitly frozen as
  `task=routing`, included in service coverage, and regression-tested for both
  positive acceptance and `native-service` failure classification. The new
  create-only runner binds 29 source files, requires clean synchronized `main`,
  revalidates both immutable failures and the corrected runtime before its sole
  new-manifest read, and forbids retries, replacements, training, development
  selection, or post-result selection. That source boundary was committed and
  pushed as `07b8967fa3d287bd6f7e8ca6bb61f27a5a013a69` before access. Its sole
  execution then read the manifest once and retained all 42 cases in order with
  42 fresh evaluator processes, 42 fresh native processes, no retry, no
  replacement, and no post-result selection. Every learned action, native case,
  opponent, broad probe, climate, map size, program, service mode, and G15-G21
  retention check passes; all nine failure counts are zero. Learned mean return
  is `1.54319060396`; the lower paired 95-percent confidence bounds over seeded-
  random-legal and wait-only are `0.390291622196` and `1.42192802898`.
  Independent offline and live validation reports
  `V2_M22_FOLLOWUP_V2_EVIDENCE=PASS cases=42 failures=0`; evidence report SHA-256
  is `032d66ec189840ed727ace92694704e55cb14e4c46963a1c6bc3a8950d613bac`
  and the retained evidence file SHA-256 is
  `21e53fa3c7f7f5a15fcd9f199f0a59920082f3e03b8292ed968da44e9dc319ec`.
...
- The first M23 implementation foundation exports both exact selected checkpoints
  ...
  requirement. The next package foundation exact-copies both checkpoint
  inventories and builds both six-file deployment packages twice with identical
  bytes and package IDs. An inference-only C++ loader accepts both packages, and
...
and G22 does not claim universal victory. The selected checkpoint was frozen
and qualified before final access and remains unchanged after the two retained
failed suites and the accepted independent suite. The retained final-v1 runtime,
...
fast, contract, and live full workflows and their claim boundaries. A clean clone
can run fast without submodules or artifacts. After
`git submodule update --init --recursive`, that same clean checkout can run
contract. Neither tier validates retained live evidence. Full is the no-argument
default and fails closed before commands unless its absolute artifact root,
manifest roles, tools, source, and nested inputs are complete.
...
source-integrated runtime, and visible-controller source coverage. Offline M23
semantic and package checks are not retained-live validation. A complete live
tree remains necessary before full can reach its unchanged 235-test V1
